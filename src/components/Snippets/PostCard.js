...
await axios.get(`${url}/users/`).then(async (response)=>{
    ...
    for(let i = 0; i < len; i++)
    {
        ...
        if(accountFind === localStorage.getItem("walletAddress"))
        {
            ...
            // setGet(get[i]);
            let getData = (get[i]);
            getData.profileURL = "App opted in Launchpad";
            getData.profileName = parseInt(getData.profileName) + 1;
            console.log(getData);
...
await axios.get(`${url}/users/`).then(async (response)=>{
    ...
    for(let i = 0; i < len; i++)
    {
        ...
        if(accountFind === localStorage.getItem("walletAddress"))
        {
            ...
            // setGet(get[i]);
            let getData = (get[i]);
            getData.asset1Name = "ELEM asset opted in Launchpad";
            getData.profileName = parseInt(getData.profileName) + 1;
            console.log(getData);
            // let postData = getData.replace('"profileName": null,','"profileName": "postCheck",');
            // console.log()
...
await axios.get(`${url}/users/`).then(async (response)=>{
    ...
    for(let i = 0; i < len; i++)
    {
        ...
        if(accountFind === localStorage.getItem("walletAddress"))
        {
            ...
            // setGet(get[i]);
            let getData = (get[i]);
            getData.twitterName = "Donated in Launchpad";
            getData.profileName = parseInt(getData.profileName) + 1;
            console.log(getData);
            // let postData = getData.replace('"profileName": null,','"profileName": "postCheck",');
            // console.log()
            ...
            ><><ToastContainer position='top-center' draggable = {false} transition={Zoom} autoClose={8000} closeOnClick = {false}/></>
                ...
                <Modal.Body>
                    ...
                    <div className="d-flex mb-20 flex-wrap align-items-start justify-content-between">
                        <div>
                            <strong>Round</strong>
                            <div className="h5 mb-0">Sale in Progress</div>
                        </div>
